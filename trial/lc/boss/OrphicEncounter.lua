...
local THUNDER_THRALL  = 214383   -- combatRoute: ACTION_RESULT_BEGIN â†’ Xoryn jump; timer 25.5s / 8s first
local LIGHTNING_FLOOD = 214355   -- combatRoute: ACTION_RESULT_BEGIN â†’ Xoryn cone; timer 21.5s / 3s first
local COLOR_CHANGE    = 213913   -- combatRoute: ACTION_RESULT_EFFECT_GAINED â†’ mirror switch alert
local BREAKOUT        = 220185   -- combatRoute: ACTION_RESULT_BEGIN â†’ crystal prison on player
local SHIELD_THROW    = 221945   -- combatRoute: ACTION_RESULT_BEGIN â†’ Crystal Sentinel caAlertCast
local XORYN_IMMUNE_1  = 217987   -- combatRoute: ACTION_RESULT_EFFECT_GAINED / FADED â†’ Xoryn away / returned
local XORYN_IMMUNE_2  = 219545   -- combatRoute: ACTION_RESULT_EFFECT_GAINED / FADED â†’ Xoryn away variant
...
local function handleLightningFlood(self, context, alerts, abilityId,
    ...
    alerts:showAction("Lightning Flood â†’ " .. target)
end
...
local function handleShieldThrow(self, context, alerts, abilityId,
    ...
    CA.alertCast(abilityId, "Shield Throw â†’ " .. target, dur, COL_LIGHTNING)
end
